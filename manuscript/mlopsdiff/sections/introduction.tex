...
Machine learning (ML) is a subset of AI\DIFaddbegin \DIFadd{, }\DIFaddend and its techniques involve the use of high-quality data. \DIFdelbegin \DIFdel{In }\DIFdelend ML logic is not explicitly programmed but  \DIFdelbegin \DIFdel{is rather }\DIFdelend learned from data. The development of industrial ML-enabled software systems involves ML pipelines that consist of several interlocking steps. To support the different steps, end-to-end in one environment, ML platforms like TensorFlow Extended (TFX) \cite{Baylor2017}, have been proposed to ensure increased automation across the steps.

Since industrial ML pipelines can be complex, \DIFdelbegin \DIFdel{it is important to gain an understanding of their characteristics }\DIFdelend \DIFaddbegin \DIFadd{understanding their characteristics is important}\DIFaddend . In a large data-intensive organization \DIFdelbegin \DIFdel{, }\DIFdelend like Google, \DIFdelbegin \DIFdel{all }\DIFdelend \DIFaddbegin \DIFadd{some }\DIFaddend 3000 ML pipelines comprising \DIFdelbegin \DIFdel{of }\DIFdelend over 450,000 trained ML models continuously update the models at least seven times a day \cite{Doris2021MLPipelines}. %A one-off ML pipeline is typically initiated to produce trained ML models ‘on-demand’, whereas a continuous ML pipeline continuously updates the models in production \cite{Baylor2019}. 
...
\DIFdelbegin \DIFdel{, often, data is constantly being generated and }\DIFdelend %DIF > , often, data is constantly being generated, and 
the performance of models \DIFdelbegin \DIFdel{can deteriorate overtime }\DIFdelend \DIFaddbegin \DIFadd{deteriorates over time }\DIFaddend \cite{Sculley2015}.
...
\begin{itemize}
    ...
    }\DIFaddend \item Tool adoption in ML pipelines (Section \ref{sec:tools})\DIFdelbegin \DIFdel{and areas to address in }\DIFdelend \DIFaddbegin \DIFadd{, and 
    }\item \DIFadd{Identification of areas that need }\DIFaddend future research (Section \ref{sec:discussion})\DIFaddbegin \DIFadd{.
