...
Machine learning (ML) is a subset of AI\DIFaddbegin \DIFadd{, }\DIFaddend and its techniques involve the use of high-quality data. \DIFdelbegin \DIFdel{In }\DIFdelend ML logic is not explicitly programmed but is rather \DIFdelbegin \DIFdel{learned }\DIFdelend \DIFaddbegin \DIFadd{earned }\DIFaddend from data. The development of industrial ML-enabled software systems involves ML pipelines that consist of several interlocking steps. To support the different steps, end-to-end in one environment, ML platforms like TensorFlow Extended (TFX) \cite{Baylor2017}, have been proposed to ensure increased automation across the steps.
...
\begin{itemize}
    ...
    }\DIFaddend \item Tool adoption in ML pipelines (Section \ref{sec:tools})\DIFdelbegin \DIFdel{and areas to address in }\DIFdelend \DIFaddbegin \DIFadd{, and areas of }\DIFaddend future research (Section \ref{sec:discussion})\DIFaddbegin \DIFadd{.
