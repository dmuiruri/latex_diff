...
Model training is conducted in public cloud settings for most cases, while a few cases train on-premises. To consistently provision training environments, 'infrastructure-as-code' practices, \DIFdelbegin \DIFdel{using tools like Terraform }\DIFdelend \DIFaddbegin \DIFadd{tools such as Terraform are used }\DIFaddend (Cases A and E).
\DIFdelbegin \DIFdel{Inference serving is either done in batch or online format.
}\DIFdelend 
...
\DIFdelbegin \DIFdel{The majority }\DIFdelend \DIFaddbegin \DIFadd{Generally, inference serving is either done in batch or online format. Majority }\DIFaddend of the models are deployed as REST (\DIFdelbegin \DIFdel{Representational }\DIFdelend \DIFaddbegin \DIFadd{REpresentational }\DIFaddend State Transfer) API endpoints on public cloud or \DIFdelbegin \DIFdel{on-premises }\DIFdelend \DIFaddbegin \DIFadd{on-premise }\DIFaddend servers. Other deployment targets include embedding the model on the actual application, such as a mobile application (Case P) or deploying to IoT devices through over-the-air deployments or onsite installations.

Models with strict inference latency requirements are deployed as gRPC (Google Remote Procedure Call) API endpoints. \DIFdelbegin \DIFdel{Case }\DIFdelend \DIFaddbegin \DIFadd{For example, case }\DIFaddend C business application has strict latency requirements and \DIFaddbegin \DIFadd{therefore }\DIFaddend uses the gRPC, which supports streaming.
...
After model deployment, monitoring is performed at different levels of granularity. The most common form of monitoring is undertaken for infrastructure management. Logging, monitoring, and alerting \DIFdelbegin \DIFdel{services and }\DIFdelend tools, like \DIFdelbegin \DIFdel{Prometheuse }\DIFdelend \DIFaddbegin \DIFadd{Prometheus }\DIFaddend and Kubernetes logging (stackdriver), are used to collect a cluster's performance metrics. \DIFdelbegin \DIFdel{Metrics }\DIFdelend \DIFaddbegin \DIFadd{These metrics }\DIFaddend are then visualized on dashboards using tools like Grafana, Tableau, or other business analytics tools. For models deployed as \DIFdelbegin \DIFdel{API, model logging (e.g., model predictions) integration with services like }\DIFdelend \DIFaddbegin \DIFadd{APIs, model metrics are logged and stored in }\DIFaddend Elasticsearch and BigQuery \DIFdelbegin \DIFdel{can be }\DIFdelend \DIFaddbegin \DIFadd{where the logs are }\DIFaddend used to perform model health and quality checks in production, e.g., \DIFdelbegin \DIFdel{average accuracy on sampled log }\DIFdelend \DIFaddbegin \DIFadd{monitoring average accuracy levels }\DIFaddend (Cases A, F, and O).
\DIFdelbegin \DIFdel{Maintaining }\DIFdelend \DIFaddbegin 

\DIFadd{Overall, maintaining }\DIFaddend the collective set tools used across a pipeline can develop into a complex task, especially when dealing with NN architectures.
